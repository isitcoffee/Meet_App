...
class MeetupController {
  ...
  async update(request, response) {
    ...
    if (meetup.past) {
      return response
        .status(400)
        .json({ error: 'cannot update past meetups.' });
    }
  ...
  async delete(request, response) {
    ...
    if (meetup.user_id !== user_id) {
      return response.json(401).json({ error: 'Not authorized' });
    }

    if (meetup.past) {
      return response.status(400).json({ error: 'cannot delete past meetups' });
    }
